...
class ClientReferralsDomain {
	...
	async getClientReferrals(CTX, value) {
		let client = {}
		const clientMongo =
			await this.clientRepository.getClientByTelegramIdInMongo(CTX.from.id)

		const authClient = await this.clientRepository.getClientWithReferrals(
			clientMongo.client_id,
			clientMongo.auth.access_token
		)
